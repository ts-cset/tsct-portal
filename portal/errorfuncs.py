...
def keep_prev_info(action, *args): # action is checked for what its being used for, *args is any number of other arguments
...
def validate_query(action, course_id, *query_args):
    ...
    if len(query_args[2]) > 4: # If the course major input was given more than 4 letters, append an error to errors.
        maj_error = "Course Majors can only have a maximum of 4 letters."
        errors.append(maj_error)

    if int(query_args[3]) > 4 or int(query_args[3]) < 0: # If course credits input was given less than 1 or more than 4, append error to errors.
        cred_error = "A course can only have between 1 and 4 credits."
        errors.append(cred_error)

    if len(query_args[4]) > 200: # if course description input was given more than 200 characters, append an error to errors.
    ...
    if errors != []: # If there are errors in errors, flash them, keep previous input info in session, and reload the page.
        for error in errors:
            ...
            if action == 'edit': # If on edit view of courses module.
                keep_prev_info(action, query_args[0], query_args[1], query_args[2], query_args[3], query_args[4])
                return render_template('portal/editcourse.html', course=query_args[5]) # query_args[5] is the course's original info.
